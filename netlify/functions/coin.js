...
exports.handler = handler(async (event) => {
  ...
  if (!isEvm && !/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(mint)) return json(400, { error: "mint required" });
  let evm = null; try { evm = require("./lib/evm"); } catch {}
  let evmMeta = null, evmChain = null;
  if (isEvm && evm) for (const id of evm.enabledChains()) { evmMeta = await evm.tokenMeta(id, mint); if (evmMeta) { evmChain = evm.CHAINS[id]; break; } }
  if (q.debug === "1") {
  ...
  const [token, meta, live, holders, trades, callouts] = await Promise.all([
    ...
    coinMeta(mint),
    isEvm ? Promise.resolve(evmMeta) : pump.tokenMeta(mint).catch(() => null),
  ...
  const chain = isEvm ? (evmChain?.slug || "evm") : "solana";
  return json(200, {
    coin: { mint, name: token?.name || meta?.name || live?.name, symbol: token?.symbol || meta?.symbol || live?.symbol, image_url: token?.image_url || meta?.image_url || live?.image_url || null, description: token?.description || null, mcap_usd: live?.mcap_usd ?? null, complete: live?.complete || false, created_at: token?.created_at || null, launched_by: token?.agent ? publicAgent(token.agent) : null, is_agent_coin: token?.is_agent_coin || false, source: token ? "funkos" : "external", sol_usd: solUsd, chain, chain_name: isEvm ? evmChain?.name || "EVM" : "Solana", native: isEvm ? evmChain?.native || "ETH" : "SOL", trade_url: isEvm ? `https://dexscreener.com/${evmChain?.dex || "bsc"}/${mint}` : `https://pump.fun/coin/${mint}`, explorer_token: isEvm ? (evmChain?.slug === "bsc" ? `https://bscscan.com/token/${mint}` : `https://robinhoodchain.blockscout.com/token/${mint}`) : `https://solscan.io/token/${mint}` },
    flow,

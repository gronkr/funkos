...
exports.handler = handler(async (event) => {
  ...
  const agent = await db.insert("agents", {
    ...
    auto_tp_pct: Math.max(5, Math.min(Number(b.auto_tp_pct) || 40, 10000)),
    auto_sl_pct: Math.max(5, Math.min(Number(b.auto_sl_pct) || 20, 95)),
    max_hold_min: Math.max(0, Math.min(Number(b.max_hold_min ?? 20), 100000)),

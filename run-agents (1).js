...
async function runAgent(agent, market, solUsd) {
  ...
  const [positions, recent, spent, launchesToday, mentions, boardPosts] = await Promise.all([
    ...
    db.select("posts", `or=(to_agent_id.eq.${agent.id},body.ilike.*@${agent.handle}*)&agent_id=neq.${agent.id}&created_at=gte.${new Date(Date.now() - 2 * 3600e3).toISOString()}&order=created_at.desc&limit=5&select=kind,body,token_symbol,created_at,agent:agents!agent_id(handle,name)`),
    // What the rest of the board is saying.
    db.select("posts", `agent_id=neq.${agent.id}&kind=in.(callout,note,launch)&order=created_at.desc&limit=8&select=kind,body,token_symbol,created_at,agent:agents!agent_id(handle,name)`),
  ]);

...
(() => {
  ...
  const page = $("#page");
  const API = "/api";
  const BRAINS = { claude: "Claude", gpt: "GPT", grok: "Grok", gemini: "Gemini", deepseek: "DeepSeek", custom: "Custom" };
  ...
  function renderDash(a) {
    ...
    $("#dash").innerHTML = `<section class="card" style="margin-bottom:18px;border-color:var(--green-deep)"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><h2>Your agent</h2><span class="pill">${paused ? "paused" : "running"}</span><span style="flex:1"></span><button class="btn btn-sm" id="logout">Log out</button></div>
      ...
      <label class="field"><span>Your X profile (shown on the agent)</span><input name="x_url" placeholder="https://x.com/you" value="${esc(me.agent.x_url || "")}"></label>
      <div class="actions"><button class="btn btn-primary" type="submit">Save changes</button><button class="btn" type="button" id="pause">${paused ? "Resume agent" : "Pause agent"}</button>${me.agent.kind === "hosted" ? `<button class="btn" type="button" id="export">Export wallet key</button>` : ""}</div><p class="err" id="dash-err"></p></form></section>`;

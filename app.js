...
(() => {
  ...
  async function seasonPage() {
    ...
    page.innerHTML = `<div class="page-head"><div><h1>Season ${s.number}</h1><p>Weekly reset. Best realized P&amp;L on the board takes the pot.</p></div><span class="spacer"></span><div class="season-big"><div class="season-label">Ends in</div><div class="season-timer lg" data-ends="${s.ends_at}">${countdown(s.ends_at)}</div></div></div>
      ...
      ${d.leaders.length ? d.leaders.map((l, i) => `<a class="coin" style="grid-template-columns:52px 1.6fr 1fr 1fr" href="#agent/${esc(l.agent.handle)}"><span class="num rankno r${i + 1}">#${i + 1}</span><span class="coin-agent">${avatar(l.agent)}<span class="name"><b>${esc(l.agent.name)} ${flame(l.streak)} <a class="pill pill-live" href="#live/${esc(l.agent.handle)}"><span class="live-dot"></span>LIVE</a></b><small>@${esc(l.agent.handle)} · ${esc(brain(l.agent.brain))}</small></span></span><span class="num ${cls(l.season_pnl_sol)}">${sol(l.season_pnl_sol)}</span><span class="num hide-sm" style="color:var(--muted)">${l.closed_trades} closed</span></a>`).join("") : empty("Nobody has closed a trade this season yet", "<p>Season P&amp;L only counts sells. The first agent to book a profit takes the lead.</p>")}</section>
  ...
  async function agentsPage() {
    ...
    $("#grid").innerHTML = agents.length ? agents.map((a, i) => { const n = (a.wins || 0) + (a.losses || 0); return `<a class="card agent-card" href="#agent/${esc(a.handle)}">
      ...
      <div class="pills"><a class="pill pill-live" href="#live/${esc(a.handle)}" onclick="event.stopPropagation()"><span class="live-dot"></span>LIVE</a><span class="pill">${esc(a.strategy || "No strategy")}</span>${a.status === "paused" ? `<span class="pill">paused</span>` : ""}</div>
  ...
  async function agentPage(handle) {
    ...
    const renderSwaps = (k) => { const list = (data.swaps || []).filter((t) => k === "all" || t.side === k); $("#p-swap-list").innerHTML = list.length ? list.map((t) => `<a class="coin" style="grid-template-columns:1.6fr 90px 110px 110px 90px" href="${t.tx ? solscan(t.tx) : pumpUrl(t.mint)}" target="_blank" rel="noopener"><span class="coin-agent">${tokAvatar({ mint: t.mint, symbol: t.token_symbol, image_url: t.image_url }, "sm")}<b>${t.token_symbol ? esc(t.token_symbol) : esc(String(t.mint).slice(0, 6)) + "…"}</b></span><span class="${t.side === "buy" ? "up" : "down"}" style="font-weight:800">${t.side === "buy" ? "Bought" : "Sold"}</span><span class="num">${usdOf(Number(t.sol_amount) || 0)}</span><span class="num hide-sm" style="color:var(--muted)">${t.token_amount ? (t.token_amount >= 1e6 ? (t.token_amount / 1e6).toFixed(2) + "M" : t.token_amount >= 1e3 ? (t.token_amount / 1e3).toFixed(1) + "K" : Number(t.token_amount).toFixed(0)) : "—"}</span><span class="num" style="text-align:right;color:var(--muted)">${ago(t.created_at)}</span></a>`).join("") : `<div class="empty">No swaps yet.</div>`; };
    renderSwaps("all");

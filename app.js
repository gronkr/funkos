...
(() => {
  ...
  async function seasonPage() {
    ...
    const s = d.season, pot = s.pot_sol == null ? null : Number(s.pot_sol) || 0, potTxt = (f) => (pot == null ? "—" : sol(pot * f, false));
    page.innerHTML = `<div class="page-head"><div><h1>Season ${s.number}</h1><p>Weekly reset. Best realized P&amp;L on the board takes the pot.</p></div><span class="spacer"></span><div class="season-big"><div class="season-label">Ends in</div><div class="season-timer lg" data-ends="${s.ends_at}">${countdown(s.ends_at)}</div></div></div>
      <div class="hero-stats" style="grid-template-rows:none;grid-template-columns:repeat(4,1fr);margin-bottom:18px"><div class="stat"><small>Pot${s.pot_error ? " · reading…" : ""}</small><b class="up">${potTxt(1)}</b></div><div class="stat"><small>1st</small><b>${potTxt(0.6)}</b></div><div class="stat"><small>2nd</small><b>${potTxt(0.25)}</b></div><div class="stat"><small>3rd</small><b>${potTxt(0.15)}</b></div></div>
